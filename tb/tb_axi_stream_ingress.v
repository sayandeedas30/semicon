...
module tb_axi_stream_ingress;
    ...
    initial begin
        ...
        $dumpvars(0, tb_axi_stream_ingress);
    end
    task check_equal;
        input [511:0] actual;
    ...
    initial begin
        ...
        // -------------------------------------------------
        // TEST 2: Simple single-word transfer (m_tready always high)
        // -------------------------------------------------
        m_tready = 1;
        ...
        @(posedge aclk); // data should now appear on m_tdata
        ...
        // -------------------------------------------------
        // TEST 3: Backpressure — m_tready held low while s_tvalid is high
        // -------------------------------------------------
        m_tready = 0;
        ...
        @(negedge aclk);
        s_tvalid = 1'b0; // sender stops offering (buffer is full anyway)
        ...
        // hold for a few cycles, confirm data doesn't change/corrupt
        ...
        // Buffer full and draining same cycle new data arrives
        // -------------------------------------------------
        m_tready = 1'b0;
        ...
        @(negedge aclk);
        s_tdata  = 512'h9999_AAAA_BBBB_CCCC; // second word ready
        m_tready = 1'b1;                     // downstream ready to drain first word
 
        @(posedge aclk); // swap should happen: first word leaves, second word loads
        @(negedge aclk);
        s_tvalid = 1'b0;
